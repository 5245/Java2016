...
public class ConsistentHashDemo {
    ...
    // 虚拟节点信息
    private static TreeMap<Integer, VisualCacheNode> visualNodeMap     = new TreeMap<>();
    // 真实节点列表
    private static List<CacheNode>                   realNodeList      = new ArrayList<>();
    // 每个真实节点对应的虚拟节点个数
    private static int                               SINGLE_VISUAL_NUM = 5;

    // 每个cacheNode上存储的数据，此处用Map做存储。key为Node的基本信息。实际上应该为Memcahe或者redis，此处用Map代替
    private static Map<String, CacheNode>            dataCache         = new HashMap<>();

    // 记录每台Cache节点上存储的key值信息。真实机器上这部分就不需要在这里存储了，这里仅用作后面做数据迁移的时候展示用。
    //key为虚拟节点的名称，value为当前节点上存储的数据信息
    private static Map<String, List<String>>         vNodeCacheDataMap = new HashMap<>();
    ...
    private static VisualCacheNode getVisualCacheNode(String dataKey) {
        ...
        if (vnodes == null || vnodes.size() == 0) {
            ...
            System.out.println(dataKey + " 未找到虚拟节点，当前hashkey=" + dataKeyHash + ",使用最小节点存储：" + nearestCacheNode.getNodeName());
        } else {
    ...
    private static void initRadomData2VisualNode() {
        for (int i = 0; i < 100; i++) {
            ...
            //            System.out.println("当前测试数据key=" + dataKey + ",使用虚拟节点：" + nearestCacheNode.getNodeName() + ",真实Cache节点："
    ...
    private static List<VisualCacheNode> genVisualCacheNode(CacheNode cacheNode) {
        List<VisualCacheNode> visualCacheNodeList = new ArrayList<>();
