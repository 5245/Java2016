...
#foreach($po in $!{columnDatas})    
	       ...
	       <td data-title="'操作'">
	       		<div class="ng-table-operator">
		       	   <a ui-sref="${lowerNames}_edit({${lowerName}Id:${lowerName}.id})" class="icons icon-edit" title="编辑"></a>
		           <a href="javascript:;" data-template="html/${entityPackage}/dialog/${className}Dialog.html" bs-modal="modal" class="icons icon-edit" title="编辑"></a>
		           <a href="javascript:;" ng-click="remove${className}(${lowerName});" class="icons icon-delete" title="删除"></i>
	       		</div>
	       </td>
	    </tr>
	</table>
