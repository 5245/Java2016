...
/**
 * MD5 + salt(随机数) 算法，用于存储密码及其验证，生成56位的密文
 * @author Jin Song
 *
 */
public class MD5Utils {
     ...
     */
    public static boolean validatePassword(String password, String passwordInDb) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        boolean validateResult = false;
        if (password != null && passwordInDb != null) {
            //将16进制字符串格式口令转换成字节数组  
    ...
    public static String getEncryptedPwd(String password) throws NoSuchAlgorithmException, UnsupportedEncodingException {
        ...
        if (password != null) {
            ...
            encryptedPwd = byteToHexString(pwd);
        }
        return encryptedPwd;
    }
